...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($mdp !== $mdp2) {
        ...
        // Vérifier si l'email est dans la liste des emails bannis
        $ban_query = $db->prepare("SELECT * FROM ban WHERE emailban = :email");
        $ban_query->execute(array(':email' => $mail));
        $ban_count = $ban_query->rowCount();

        if ($ban_count > 0) {
            // Redirection vers la page index.php si l'email est banni
