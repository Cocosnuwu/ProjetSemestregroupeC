document.addEventListener('DOMContentLoaded', function () {
  ...
  searchInput.addEventListener('input', function () {
    ...
    if (query.length > 0) {
      fetch(`searchindex.php?search=${query}`) // Modifier le chemin vers votre fichier PHP
        ...
        .then(data => {
          ...
          if (data.length > 0) {
            data.forEach(prof => {
              resultsHtml += `<p><a href="https://rencontreprof.000webhostapp.com/www/voirvisite.php?user_id=${prof.id}">${prof.nom} ${prof.prenom}</a></p>`; // Modifier le lien
